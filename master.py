...
LOGS_TASK={}
LOGS_JOB={}
with open(sys.argv[1]+".txt","w") as CUR_FILE:
    CUR_FILE.write(sys.argv[1]+"\n")
def print_worker_state():
    with open(sys.argv[1]+".txt","a") as CUR_FILE:
        for i in range(len(WORKERS_MATRIX)):
            CUR_FILE.write(str(sum(WORKERS_MATRIX[i])))
            if i==len(WORKERS_MATRIX)-1:
                CUR_FILE.write(" "+str(time.time())+"\n")
...
def getting_job_request(lock):
    ...
    while True:
        ...
        if msg:
            msg=json.loads(msg)
            #dee+=len(msg["map_tasks"])+len(msg["reduce_tasks"])
            #print(dee,"deee")
            lock.acquire()
...
def task_updation(lock):
    ...
    while True:
        ...
        if DATA:
            ...
            curtime=time.time()
            with open(sys.argv[1]+".txt","a") as CUR_FILE:
                CUR_FILE.write(str(DATA["job"])+" "+str(DATA["task"])+" "+str(curtime-LOGS_TASK[DATA["job"]][int(DATA["task"])])+"\n")
            print("wrote this line",DATA["job"]+" "+str(DATA["task"])+" "+str(curtime-LOGS_TASK[DATA["job"]][int(DATA["task"])]))
            del LOGS_TASK[DATA["job"]][int(DATA["task"])]
            print(DATA["map_red"],"MAP_RED",DATA["task"],"DATA_TASK",JOB_MATRIX[DATA["job"]][2],"JOB_MATRIX_2")
            if int(DATA["map_red"])==2 and int(DATA["task"])==JOB_MATRIX[DATA["job"]][2]-1:
                with open(sys.argv[1]+".txt","a") as CUR_FILE:
                    CUR_FILE.write(str(DATA["job"])+" "+str(curtime-LOGS_JOB[DATA["job"]])+"\n")
                print("wrote this line",DATA["job"]+" "+str(curtime-LOGS_JOB[DATA["job"]]))
                del LOGS_JOB[DATA["job"]]
            WORKERS_MATRIX[WORKER_INDEX[DATA["worker"]]][int(DATA["slot"])]=0
    ...
    while True:
        ...
        for j in range(len(WORKERS_MATRIX[i])):
            if WORKERS_MATRIX[i][j]==0:
                ...
                curtime=time.time()
                if send["job_id"] not in LOGS_JOB:
                    LOGS_JOB[send["job_id"]]=curtime
                    LOGS_TASK[send["job_id"]]={}
                
                LOGS_TASK[send["job_id"]][int(send["task_id"])]=curtime
    ...
    while True:
        ...
        for l in range(TRASH_DICT["NO_WORKERS"]):
            if 0 in WORKERS_MATRIX[l]:
                if (len(WORKERS_MATRIX[l])-sum(WORKERS_MATRIX[l]))>max_sum:
                    i=l
                    max_sum=len(WORKERS_MATRIX[l])-sum(WORKERS_MATRIX[l])
        for j in range(0,len(WORKERS_MATRIX[i])):
            if WORKERS_MATRIX[i][j]==0:
                #print(send,"sending to",i,j)
                curtime=time.time()
...
SENDING_SOCKET=socket.socket(socket.AF_INET,socket.SOCK_STREAM)aaa
